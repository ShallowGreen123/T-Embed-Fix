...
inline const char* stateTitle() {
    switch (gState) {
        case State::INIT_MIC:       return "Initializing microphone";
        case State::MIC_TEST:       return "Microphone capture test";
        case State::INIT_SPK:       return "Preparing speaker output";
        case State::SPEAKER_TEST:   return "Speaker tone sweep";
        case State::INIT_LOOPBACK:  return "Configuring loopback";
        case State::LOOPBACK_TEST:  return "Mic to speaker loopback";
        case State::DONE_PASS:      return "Audio self-check passed";
        case State::DONE_FAIL:      return "Audio self-check failed";
...
inline const char* stateHint() {
    switch (gState) {
        case State::INIT_MIC:       return "Starting the PDM mic path...";
        case State::MIC_TEST:       return "Make some noise close to the microphone.";
        case State::INIT_SPK:       return "Enabling I2S and amplifier...";
        case State::SPEAKER_TEST:   return "Listen for the three test tones.";
        case State::INIT_LOOPBACK:  return "Bringing mic and speaker online together...";
        case State::LOOPBACK_TEST:  return "Speak now. You should hear delayed playback.";
        case State::DONE_PASS:      return "All three stages completed successfully.";
        case State::DONE_FAIL:      return "Check mic, speaker and power path, then rerun.";
        default:                    return "";
    }
}
...
inline uint32_t activeStageDurationMs() {
    switch (gState) {
        case State::MIC_TEST:      return kMicTestDurationMs;
        case State::SPEAKER_TEST:  return kSpkFreqDwellMs * 3;
        case State::LOOPBACK_TEST: return kLoopbackDurationMs;
        default:                   return 0;
    }
}
...
inline StageStatus micStageStatus() {
    if (gState == State::INIT_MIC || gState == State::MIC_TEST) return StageStatus::ACTIVE;
    if (!gMicStarted) return StageStatus::PENDING;
    return gMicOk ? StageStatus::PASS : StageStatus::FAIL;
}

inline StageStatus spkStageStatus() {
    if (gState == State::INIT_SPK || gState == State::SPEAKER_TEST) return StageStatus::ACTIVE;
    if (!gSpkStarted) return StageStatus::PENDING;
    return gSpkOk ? StageStatus::PASS : StageStatus::FAIL;
}

inline StageStatus loopStageStatus() {
    if (gState == State::INIT_LOOPBACK || gState == State::LOOPBACK_TEST) return StageStatus::ACTIVE;
    if (!gLoopStarted) return StageStatus::PENDING;
...
inline uint16_t meterBarColor(uint16_t width, uint16_t fullWidth) {
    if (width < fullWidth * 60 / 100) return TFT_GREEN;
    if (width < fullWidth * 85 / 100) return TFT_YELLOW;
    return TFT_RED;
}

inline const char* levelLabel(uint16_t rms) {
    if (rms < 100) return "SILENT";
    if (rms < 500) return "LOW";
    if (rms < 2000) return "MED";
    return "LOUD";
}

inline uint16_t levelColor(uint16_t rms) {
    if (rms < 100) return TFT_DARKGREY;
    if (rms < 500) return TFT_CYAN;
    if (rms < 2000) return TFT_GREEN;
    return TFT_RED;
}
...
void drawProgressBar(Canvas& gfx, int16_t x, int16_t y, int16_t w, int16_t h) {
    ...
    const uint32_t duration = activeStageDurationMs();
    if (duration == 0 || w <= 2 || h <= 2) return;

    const int16_t fillW = (int16_t)((uint32_t)(w - 2) * activeStageElapsedMs() / duration);
    if (fillW > 0) {
...
void drawVuBar(Canvas& gfx, int16_t x, int16_t y, int16_t w, int16_t h, uint16_t rms, uint16_t peak) {
    ...
    int16_t fillW = (int16_t)((uint32_t)rms * w / 8000);
    if (fillW > w) fillW = w;
    ...
    int16_t peakX = (int16_t)((uint32_t)peak * w / 8000);
    if (peakX > w) peakX = w;
    if (peakX > 0) {
