...
inline String statusDetailText() {
  ...
  switch (uiState) {
    case UiState::Init:         return "Bringing up display and NFC reader";
...
inline const char* footerText() {
  switch (uiState) {
    case UiState::CardFound:    return "Hold tag steady to keep this snapshot";
    case UiState::IdentifyFail: return "Re-center tag or try another NFC-A card";
    case UiState::FatalError:   return "Reader halted";
    case UiState::NoCard:
    case UiState::Scanning:
    case UiState::Init:
    default:                    return "Polling every 180 ms | Lost timeout 700 ms";
  }
}
...
void drawCardDetails(Canvas& gfx) {
  ...
  drawFieldCard(gfx, kMetaRightX, kMetaY, kMetaW, kMetaH, "ATQA/SAK",
                currentCard.atqa || currentCard.sak ? atqaSakText() : "-", kColorPanelEdge);
  drawFieldCard(gfx, kSizeX, kSizeY, kSizeW, kSizeH, "User/Total", sizeText(),
                currentCard.totalSize ? kColorPanelEdge : TFT_DARKGREY);
}
